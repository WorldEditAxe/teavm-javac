 ...
 * GUI problems with desktop toolkits. In the case of the default renderer, the
...
//public class PApplet extends PSketch {  // possible in the next alpha
  static private PlatformRuntime platformRuntime() {
    return PlatformRuntimeProvider.get();
  }


  /** Full name of the Java version (i.e. 1.5.0_11). */
  static public final String javaVersionName =
    platformRuntime().getProperty("java.version");
  ...
  /**
   * Whether to use native dialogs for selectInput and selectOutput.
...
//  static final boolean DEBUG = true;
   ...
   * can be obtained from platform key event constants.
   *
   * @webref input:keyboard
   ...
   */
  long millisOffset = platformRuntime().currentTimeMillis();
  ...
  static public Throwable uncaughtThrowable;
  ...
  static protected boolean disableDesktopRuntime =
    platformRuntime().getProperty("processing.desktop.disable", "false").equals("true");;
  ...
  /** Disable desktop runtime initialization */
  static public final String ARGS_DISABLE_DESKTOP = "--disable-desktop";
...
//  static public final String ARGS_SPAN_DISPLAYS = "--span";
  ...
  String renderer = platformRuntime().defaultRenderer();
  ...
  void handleSettings() {
    ...
    if (!disableDesktopRuntime) {
      displayWidth = platformRuntime().getDisplayWidth();
      displayHeight = platformRuntime().getDisplayHeight();
    } else {
      // https://github.com/processing/processing4/issues/57
      System.err.println("Desktop runtime disabled, displayWidth/displayHeight will be 0");
    }
    ...
    if (display == SPAN && platform == MACOS) {
      ...
      PlatformProcess p = exec("defaults", "read", "com.apple.spaces", "spans-displays");
  ...
  */
  public int displayDensity() {
    ...
    int displayCount = 0;
    if (!disableDesktopRuntime) {
      displayCount = platformRuntime().getDisplayCount();
    } else {
      // https://github.com/processing/processing4/issues/57
      System.err.println("display count needs to be implemented for this runtime");
    }
  ...
  */
  public int displayDensity(int display) {
    if (!disableDesktopRuntime) {
      return platformRuntime().getDisplayDensity(display);
    }
    /*
    ...
    */
    // https://github.com/processing/processing4/issues/57
    System.err.println("displayDensity() unavailable because desktop runtime is disabled");
  ...
  */
   ...
   * <li>touchEvent
   * </ul>
   * Older desktop-native events are no longer supported.
...
/**
  ...
  */
  ...
  public PGraphics createGraphics(int w, int h) {
    return createGraphics(w, h, platformRuntime().defaultRenderer());
  }
  ...
  protected PGraphics makeGraphics(int w, int h,
    ...
    try {
      PGraphics pg = platformRuntime().createGraphics(renderer);
...
//      }
  ...
  public void handleDraw() {
    ...
    long now = platformRuntime().nanoTime();
 ...
 */
  ...
  protected void handleMouseEvent(MouseEvent event) {
    ...
    // no native code backing Surface objects handle it.
    ...
    /*
    // Compatibility for older code with native event object params.
    ...
    */
    ...
    if (event.getAction() == KeyEvent.PRESS) {
      ...
      if (external &&
        ...
        // Native event objects vary by renderer, so use normalized state.
        exit();
      }
    }
  }
   ...
   */
  public int millis() {
    return (int) (platformRuntime().currentTimeMillis() - millisOffset);
  }
   ...
   * @usage Application
   */
  static public PlatformProcess launch(String... args) {
    String[] params = null;
    ...
    if (platform == WINDOWS) {
      ...
      for (String launcher : launchers) {
        ...
        try {
          PlatformProcess p = platformRuntime().exec(new String[] { launcher });
   ...
   * <pre>
   * PlatformProcess p = exec("/usr/bin/say", "waiting until done");
   * try {
   *   int result = p.waitFor();
   ...
   * @return a platform process object
   */
  static public PlatformProcess exec(String... args) {
    try {
      return platformRuntime().exec(args);
    } catch (Exception e) {
  ...
  static public int shell(StringList stdout, StringList stderr, String... args) {
    ...
    if (platform == WINDOWS) {
      shell = platformRuntime().getenv("COMSPEC");
      runCmd = "/C";
    } else {
      shell = "/bin/sh";
      runCmd = "-c";
  ...
  */
  ...
  public void dispose() {
    ...
    platformRuntime().cleanupSketch(this);
...
/**
  ...
  */
  ...
  public PImage loadImage(String filename, String extension) {
    ...
    if (surface == null) {
      return platformRuntime().loadImage(this, filename, extension);
    }
...
//    }
  ...
  static protected String calcSketchPath() {
    return platformRuntime().calcSketchPath();
  }
  ...
  static public File desktopFile(String what) {
    if (desktopFolder == null) {
      desktopFolder = platformRuntime().getDesktopFolder();
  ...
  public File dataFile(String where) {
    return platformRuntime().dataFile(this, sketchPath, where);
  }
 ...
 */
  ...
  */
  ...
  static public void runSketch(final PApplet constructedSketch) {
    runSketch(constructedSketch, null);
  }


  /**
   * Run an already-created sketch instance without resolving the sketch class
   * by name. Browser runtimes use this entry point after installing their
   * platform runtime.
   */
  static public void runSketch(final PApplet constructedSketch,
                               final String[] sketchArgs) {
    if (constructedSketch == null) {
      throw new NullPointerException("constructedSketch");
    }
    ...
    platformRuntime().setProperty("java.net.useSystemProxies", "true");

    if (!disableDesktopRuntime) {
      platformRuntime().initRun();
      platformRuntime().initSketch(constructedSketch);
    }

    constructedSketch.display = -1;
    constructedSketch.present = false;
    constructedSketch.fullScreen = false;
    constructedSketch.pixelDensity = constructedSketch.displayDensity();
    constructedSketch.pixelDensityWarning = constructedSketch.pixelDensity > 1;
    constructedSketch.sketchPath = platformRuntime().calcSketchPath();
    constructedSketch.args = sketchArgs;
    constructedSketch.handleSettings();
    constructedSketch.external = false;

    final PSurface surface = constructedSketch.initSurface();
    surface.placeWindow(null, null);
  ...
  static public void runSketch(final String[] args,
    ...
    if (platform == MACOSX) {
      ...
      if (coreJarPath.endsWith("/Contents/Java/core.jar")) {
        ...
        String libraryPath = platformRuntime().getProperty("java.library.path");
        libraryPath += File.pathSeparator + javaPath;
        platformRuntime().setProperty("java.library.path", libraryPath);
      }
    }
    */
    ...
    boolean external =
      platformRuntime().getProperty("processing.external", "false").equals("true");;
    String locationProperty = platformRuntime().getProperty("processing.location", null);
    int[] location = locationProperty != null ?
      parseInt(split(locationProperty, ',')) : null;

    String editorLocationProperty =
      platformRuntime().getProperty("processing.editor.location", null);
    int[] editorLocation = editorLocationProperty != null ?
      parseInt(split(editorLocationProperty, ',')) : null;
    String name = null;
    int windowColor = 0;
    int stopColor = 0xff808080;
    boolean hideStop =
      platformRuntime().getProperty("processing.stop.hide", "false").equals("true");

    int displayNum = -1;  // use default
    boolean present =
      platformRuntime().getProperty("processing.present", "false").equals("true");
    boolean fullScreen =
      platformRuntime().getProperty("processing.fullscreen", "false").equals("true");
    float uiScale = parseInt(platformRuntime().getProperty("processing.uiScale", "0"), 0);
    ...
    while (argIndex < args.length) {
      ...
      if (equals != -1) {
        ...
        switch (param) {
          ...
          case ARGS_DISABLE_DESKTOP:
            disableDesktopRuntime = true;
            break;
    ...
    if (platform == WINDOWS) {
      ...
      if (uiScale != 0) {
        platformRuntime().setProperty("sun.java2d.uiScale", String.valueOf(uiScale));
      //} else {
    ...
    if (!disableDesktopRuntime) {
      platformRuntime().initSketch(sketch);
    }
