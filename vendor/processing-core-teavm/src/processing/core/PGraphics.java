...
//  int matrixStackDepth;
  ...
  /**
   * Native image object associated with this renderer, when a platform needs
   * one. Browser builds must not expose a desktop UI type here.
   */
  public Object image;
...
//    fillA = savedFillA;
  ...
  protected PFont createDefaultFont(float size) {
    ...
    return new PFont(defaultFontName, size * parent.pixelDensity, true, null,
                     false, parent.pixelDensity);
  }
  ...
  protected PFont createFont(String name, float size,
    ...
    if (name == null) {
      name = defaultFontName == null ? "sans-serif" : defaultFontName;
    }
...
//  }
  ...
  public final float hue(int rgb) {
    if (rgb != cacheHsbKey) {
      rgbToHsb((rgb >> 16) & 0xff, (rgb >> 8) & 0xff,
  ...
  static public int lerpColor(int c1, int c2, float amt, int mode) {
    ...
    if (mode == RGB) {
      ...
      rgbToHsb((c1 >> 16) & 0xff, (c1 >> 8) & 0xff, c1 & 0xff,
               lerpColorHSB1);
      rgbToHsb((c2 >> 16) & 0xff, (c2 >> 8) & 0xff, c2 & 0xff,
               lerpColorHSB2);
  ...
  static protected float[] rgbToHsb(int r, int g, int b, float[] hsb) {
    if (hsb == null) {
      hsb = new float[3];
    }
    ...
    float saturation = (cmax == 0) ? 0 : ((float) (cmax - cmin) / (float) cmax);
    float hue = 0;
  ...
  static protected int hsbToRgb(float hue, float saturation, float brightness) {
    int r = 0;
    int g = 0;
    int b = 0;
