...
export async function createCompiler(options: CreateCompilerOptions = {}): Promise<JavaCompiler> {
...
export async function installWorker(options: CompilerRuntimeLoadOptions = {}): Promise<CompilerRuntime> {
  const runtime = await loadCompilerRuntime(resolveCompilerRuntimeInput(options));
...
export function createJavaProgramOptions(
  options: JavaProgramOptions = {},
  openFilePath: JavaProgramFileOpenPath = openJavaFilePath
): JavaProgramOptions {
  const {
    installImports,
    stdio,
...
export async function createJavaProgram(wasmBytes: BinaryInput, options: JavaProgramOptions = {}): Promise<JavaProgram> {
...
export class JavaProgram {
  ...
  constructor(wasmProgram, options: JavaProgramOptions = {}) {
  ...
  async execute(argsOrOptions: string[] | { args?: string[]; onFinish?: (result: { program: JavaProgram }) => void; timeoutMs?: number } = []): Promise<{ program: JavaProgram }> {
...
export async function getCompilerWasmGCSupport(): Promise<{ supported: boolean; reason: string | null }> {
...
export class JavaCompiler {
  ...
  async setClasslib({ javac, runtime }: CompilerClasslibOptions): Promise<this> {
    if (javac != null) {
...
export function installJavaRuntimeFileSystem(
  imports,
  options: JavaProgramFileSystemCallbacks = {},
  openFilePath: JavaProgramFileOpenPath = openJavaFilePath
) {
...
function installGlobalJavaRuntimeFileSystem(options: JavaProgramFileSystemCallbacks = {}, openFilePath = openJavaFilePath) {
  const fileImports = createJavaProgramFileImportModule(options, openFilePath);
...
function createJavaProgramFileImportModule(options: JavaProgramFileSystemCallbacks = {}, openFilePath = openJavaFilePath) {
...
function closeFilePath(fsOptions, path, mode, data) {
  ...
  return fsOptions.onFileClose(
    normalizeStorePath(path),
    normalizeFileMode(mode),
    data == null ? null : copyFileWriteData(data)
  );
}
