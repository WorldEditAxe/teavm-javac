...
public final class Worker {
    private static final String OUTPUT_TYPE_JAVASCRIPT = "javascript";
    private static final String OUTPUT_TYPE_WEBASSEMBLY = "webassembly";
    private static final String DEFAULT_JAVASCRIPT_OUTPUT_NAME = "classes.js";
    private static final String DEFAULT_JAVASCRIPT_MODULE_TYPE = "UMD";
    private static final String DEFAULT_WEBASSEMBLY_OUTPUT_NAME = "app";
    ...
    private void compileAll(CompileMessage request) throws IOException {
        ...
        var outputType = getOutputType(request);
        var outputName = getOutputName(request, outputType);
        var outputFileName = getOutputFileName(outputType, outputName);
        ...
        response.setOutputName(outputFileName);

        if (doCompile(request) && detectMainClass(request) && generateOutput(request, outputType, outputName)) {
            response.setStatus("successful");
            response.setScript(readResultingFile(outputType, outputName));
        } else {
    ...
    private String getOutputType(CompileMessage request) {
        var target = request.getTarget();
        if (target != null && (target.equals("js") || target.equals(OUTPUT_TYPE_JAVASCRIPT))) {
            return OUTPUT_TYPE_JAVASCRIPT;
        }
        return OUTPUT_TYPE_WEBASSEMBLY;
    }

    private String getOutputName(CompileMessage request, String outputType) {
        var outputName = request.getOutputName();
        if (outputName != null && !outputName.isEmpty()) {
            return outputType.equals(OUTPUT_TYPE_JAVASCRIPT) ? outputName : getWebAssemblyOutputBaseName(outputName);
        }
        return outputType.equals(OUTPUT_TYPE_JAVASCRIPT)
                ? DEFAULT_JAVASCRIPT_OUTPUT_NAME
                : DEFAULT_WEBASSEMBLY_OUTPUT_NAME;
    }

    private static String getOutputFileName(String outputType, String outputName) {
        return outputType.equals(OUTPUT_TYPE_JAVASCRIPT) ? outputName : outputName + ".wasm";
    }

    private static String getWebAssemblyOutputBaseName(String outputName) {
        return outputName.endsWith(".wasm") ? outputName.substring(0, outputName.length() - ".wasm".length())
                : outputName;
    }
    ...
    private boolean generateWebAssembly(CompileMessage request, String outputName) {
        ...
        var options = new WebAssemblyCompilationOptions() {
            ...
            public JSString getOutputName() {
                return JSString.valueOf(outputName);
            }
    ...
    private boolean generateJavaScript(CompileMessage request, String outputName) {
        var requestId = request.getId();
        var moduleType = request.getModuleType();
    ...
    private Int8Array readResultingFile(String outputType, String outputName) {
        return outputType.equals(OUTPUT_TYPE_JAVASCRIPT)
                ? compiler.getJavaScriptOutputFile(outputName)
                : compiler.getWebAssemblyOutputFile(getOutputFileName(outputType, outputName));
    }
}
