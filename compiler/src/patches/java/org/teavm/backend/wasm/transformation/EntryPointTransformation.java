...
public class EntryPointTransformation implements ClassHolderTransformer {
    ...
    private static final ValueType JS_OBJECT = ValueType.object("org.teavm.jso.JSObject");
    private static final MethodDescriptor MAIN_EXPORT = new MethodDescriptor("main_$caller",
            STRING_ARRAY, JS_OBJECT, ValueType.VOID);
    private static final MethodReference START_MAIN = new MethodReference("org.teavm.runtime.TeaVMJavacMain",
            new MethodDescriptor("start", STRING_ARRAY, JS_OBJECT, ValueType.VOID));
    ...
    public void transformClass(ClassHolder cls, ClassHolderTransformerContext context) {
        if (cls.getName().equals(entryPoint)) {
            ...
            if (mainMethod != null) {
                generateMainMethodCaller(cls, context);
            }
    ...
    private void generateMainMethodCaller(ClassHolder cls, ClassHolderTransformerContext context) {
        var mainMethodCaller = new MethodHolder(MAIN_EXPORT);
        ...
        pe.invoke(START_MAIN, pe.var(1, STRING_ARRAY), pe.var(2, JS_OBJECT));
        pe.exit();
    }
